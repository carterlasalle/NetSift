...
def generate_demo(path: str | Path, *, force: bool = False) -> Path:
    ...
    with destination.open("wb") as stream:
        ...
        for index, packet in enumerate(packets):
            stream.write(struct.pack("<IIII", 1_700_000_000 + index, index * 100_000, len(packet), len(packet)))
            stream.write(packet)
    return destination
...
def _packets() -> list[bytes]:
    ...
    arp = struct.pack("!HHBBH", 1, 0x0800, 6, 4, 1) + mac_a + bytes([10,0,0,10]) + b"\0"*6 + bytes([10,0,0,1])
    question = b"\x07example\x03com\0" + struct.pack("!HH", 1, 1)
    query = struct.pack("!HHHHHH", 0xCAFE, 0x0100, 1, 0, 0, 0) + question
    answer = struct.pack("!HHHHHH", 0xCAFE, 0x8180, 1, 1, 0, 0) + question + b"\xc0\x0c" + struct.pack("!HHIH", 1,1,60,4) + bytes([93,184,216,34])
...
def _ip4(payload: bytes, protocol: int, source: str, destination: str) -> bytes:
    return bytes([0x45,0]) + struct.pack("!HHHBBH", 20+len(payload), 1, 0, 64, protocol, 0) + ipaddress.ip_address(source).packed + ipaddress.ip_address(destination).packed + payload
